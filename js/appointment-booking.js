...
(function(){
    ...
    deptSelect.addEventListener('change', () => {
        ...
        if(!dept){
            ...
            doctorSelect.innerHTML = '<option value="">Select a doctor</option>' + 
                dept.doctors.map(doc => `<option value="${doc}">${doc}</option>`).join('');
        }
        updateSummary();
        renderSlots();
    });

    [doctorSelect, dateInput].forEach(el => el.addEventListener('change', () => { 
        updateSummary(); 
        renderSlots(); 
    }));
    ...
    function renderSlots(){
        ...
        slotGrid.innerHTML = db.slots.map(t => `
            <button type="button" class="slot-btn ${taken.has(t) ? 'taken' : ''}" 
                    data-time="${t}" ${taken.has(t) ? 'disabled' : ''} 
                    role="radio" aria-checked="false">${t}</button>
        `).join('');
    ...
    function renderMyAppointments(){
        ...
        document.getElementById('my-appts-body').innerHTML = rows.map(a => {
            const badgeClass = a.status === 'confirmed' || a.status === 'completed' ? 'badge-good' : 
                              a.status === 'cancelled' ? 'badge-critical' : 'badge-pending';
