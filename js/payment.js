...
(function(){
    ...
    function render(){
        ...
        document.getElementById('invoice-list').innerHTML = myInvoices.map(inv => `
            <div class="invoice-row ${inv.status === 'paid' ? 'paid' : ''} ${inv.id === selectedInvoiceId ? 'selected' : ''}" 
                 data-inv="${inv.id}" ${inv.status === 'paid' ? '' : 'tabindex="0" role="button"'}>
    ...
    document.getElementById('payment-form').addEventListener('submit', (e) => {
        ...
        if(!selectedInvoiceId){ 
            showToast('Select an invoice to pay first.', 'error'); 
            return; 
        }
        ...
        if(!nameOk || !numberOk || !expOk || !cvvOk){
            showToast('Check the highlighted card details.', 'error'); 
            return;
        }
